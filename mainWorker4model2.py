...
class mainWorker4model2:
    def __init__(self, yamlfilename, datafilename, phi, resultsfilename):
        self.gamma = None
        self.inputsize = None
        ...
        self.model = model2(self.gamma, self.budget, self.phi)
        ...
        with open(self.resultsfilename, 'w') as wf:
            with open(self.datafilename) as rf:
                for line in rf:
                    ...
                    print(f"mainWorker4model2.run() Cumulative Error: {self.cumError}")
                    try:
                        self.model.learning(x, y)
                    except Exception as e:
                        print(f"mainWorker4model2.run() Exception! learning() raised for line: {e}")
