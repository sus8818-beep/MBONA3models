  ...
  for i in $(seq 10 99); do
    ...
    case "$EXECODE" in
        "mainWorker4model2.py")
            nohup python3 "$EXECODE" --yaml "$PARAM" --data "$DATA-$n.csv" --phi "$ratio" --results "$RESULT-$ratio-$n.dat" > log.txt &
            ;;
            
        "mainWorker4model1.py")
            nohup python3 "$EXECODE" --yaml "$PARAM" --data "$DATA-$n.csv" --phi "$ratio" --results "$RESULT-$ratio-$n.dat"  --rmThreshold "$RMTHRESHOLD" > log.txt &
            ;;

        "mainWorker4model3.py")
            nohup python3 "$EXECODE" --yaml "$PARAM" --data "$DATA-$n.csv" --phi "$ratio" --results "$RESULT-$ratio-$n.dat"  --spontaneousPhi "$SPONTANEOUSPHI" > log.txt &
            ;;
    esac
    throttle
  done
